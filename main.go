...
(function(){
  ...
  const generateKey=()=>{try{if(typeof crypto==='undefined'||typeof crypto.getRandomValues!=='function')throw new Error('当前浏览器不支持自动生成 API Key');const bytes=new Uint8Array(32);crypto.getRandomValues(bytes);const value='sk-'+Array.from(bytes,b=>b.toString(16).padStart(2,'0')).join('');$('key-value').value=value;$('key-value').focus();$('key-value').select();status('已生成 API Key，请填写名称和备注后保存。','ok');}catch(error){status('生成失败：'+error.message,'error')}};
  ...
  const copyKey=async index=>{const value=state.keys[index];try{if(navigator.clipboard&&navigator.clipboard.writeText){await navigator.clipboard.writeText(value)}else{const area=document.createElement('textarea');area.value=value;area.style.position='fixed';area.style.opacity='0';document.body.appendChild(area);area.select();if(!document.execCommand('copy'))throw new Error('浏览器拒绝复制');area.remove()}status('API Key 已复制。','ok')}catch(error){status('复制失败：'+error.message,'error')}};
